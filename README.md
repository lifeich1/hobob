...
| 目录 | 职责 | 详细文档 |
| --- | --- | --- |
| `hobob/` | 主应用（workspace 唯一成员 crate），warp web + 后台抓取循环（systems fetch_loop） | [hobob/README.md](hobob/README.md) |
...
- **数据文件（M1 起）**：持久化唯一来源 = `~/.hobob/state.redb`（redb + bincode，`hobob/src/store.rs`）：内存 ECS world（`db/mod.rs`）组件直写/批量 flush 落盘；**`~/bench.json` 是 v1 遗留，M1 起不再读写**（留在磁盘仅作备份，不迁移）。state 路径优先级：CLI `--state <PATH>` > 环境变量 `HOBOB_STATE` > `$HOME/.hobob/state.redb`。日志配置模板复制到 `~/log4rs.yml`（`hobob/assets/log4rs.yml`）。
...
# 运行（默认端口 3731；状态唯一来源 = state.redb，不再读写 ~/bench.json）
cargo run -p hobob
...
部署到设备：交叉编译产物 scp 到 `opi:/lintd/`，然后用 `hobob_dbgconn` 远程重启（见其 README）。⚠️ **首次部署 M1 后 state.redb 为空库（bench.json 不再读取），需重新关注 up**；旧 `~/bench.json` 保留作备份但不使用。
